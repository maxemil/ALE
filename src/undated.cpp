...
// In the loop below with 4 iterations, we calculate the mean probability mPTE for a gene to become extinct across all branches.
  ...
  for (int i=0;i<4;i++)
  {
    ...
    if (i>0) // There should be no need for this loop at the first iteration, because then it leaves mPTE_ancestral_correction at 0.
    {
      for (int e=0;e<last_branch ;e++)
      {
        ...
        for (vector<int>::iterator it=ancestors[e].begin(); it!=ancestors[e].end() ;it++)
        {
          ...
          int f=(*it);
          //if (ancestral[e][f]==1)
          mPTE_ancestral_correction[e]+=  (wT[f])*uE[f]; //That's how we forbid transfers to ancestors of a branch
        }
      }
    }
    ...
    for (int e=0;e<last_branch;e++)
    {
      ...
      {
        uE[e]= PL[e]+PD[e]*uE[e]*uE[e]+uE[e]*(mPTE- mPTE_ancestral_correction[e])/tau_norm[e];
      }
  ...
  if (not is_a_leaf)
  {
    ...
    for (int i=0;i<N_parts;i++)
    {
      ...
      if (not ancestral[e][f])
      {
        ...
        uq_resum+=uq[gp_i][e]*(wT[f]/tau_norm[e])*uq[gpp_i][f]*pp+EPSILON;
        if (r*uq_sum<uq_resum)
        {
          register_Tfrom(e);
          register_Tto(f);
